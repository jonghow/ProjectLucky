...
/// 생산 공장을 만드는 생산 패턴
/// </summary>
public class MealFactoryEntityFactory : EntityFactoryBase
{
    public override Entity CreateEntity(int _entityTID, Vector3 _position)
    {
        ResourceManager.GetInstance().GetResource(ResourceType.FactoryEntity, _entityTID, true, (_modelResource) => {
            // 여기서 나오는 obj는 리소스 매니저에서 오는 단순 월드모델 오브젝트
            var _modelObj = GameObject.Instantiate(_modelResource);
            var _actPlayer = _modelObj.GetComponent<ActionPlayer>();
            // Model Instantiate
            ...
            var _mealFactoryContoller = _obj.AddComponent<EntityMealFactoryController>();
            ...
            EntityManager.GetInstance().AddEntity(EntityDivision.MealFactory, uUID, ref _cEntity);
            // 선 컴포넌트 세팅 및 UID 발급

            _cEntity.InitEntityData(uUID, _entityTID, EntityDivision.MealFactory, _mealFactoryContoller);
            // Entity 와 Controller 연결

            _mealFactoryContoller.SetActPlayer(_actPlayer);
            _mealFactoryContoller.SetUp(uUID, _entityTID);
            // 컨트롤러 셋업

            _mealFactoryContoller.SetMoveAgent(_moveAgent);
            // 무빙 에이전트 셋업

            _moveAgent.SetPathFinder(_pathFinder);
            // Path Finder 셋업

            _moveAgent.OnInitialize(uUID, _mealFactoryContoller);
            // 필수 Entity Script 설정
            ...
            _mealFactoryContoller.AISetUp();
            // AI SetUp

            _modelObj.transform.SetParent(_obj.transform);
            // 부모로 세팅

            _mealFactoryContoller.Pos3D = _position;
            // 포지션 세팅
    ...
    public async UniTask CreateEntity(int _entityTID, Vector3 _position, Action<Entity> _onCB_Create)
    {
        ...
        ResourceManager.GetInstance().GetResource(ResourceType.MealFactoryAnimationController, _entityTID, true, (obj) =>
        {
            _IsLoaded = true;
        });
        ...
        ResourceManager.GetInstance().GetResource(ResourceType.FactoryEntity, _entityTID, true, (_modelResource) => {
            ...
            var _userContoller = _obj.AddComponent<EntityMealFactoryController>();
            ...
            _cEntity.InitEntityData(uUID, _entityTID, EntityDivision.MealFactory, _userContoller);
            // Entity 와 Controller 연결
...
/// 조합 공장을 만드는 패턴
/// </summary>
public class CombineFactoryEntityFactory : EntityFactoryBase
{
    public override Entity CreateEntity(int _entityTID, Vector3 _position)
    {
        ResourceManager.GetInstance().GetResource(ResourceType.FactoryEntity, _entityTID, true, (_modelResource) => {
            ...
            var _combineFactoryContoller = _obj.AddComponent<EntityCombineFactoryController>();
            ...
            _cEntity.InitEntityData(uUID, _entityTID, EntityDivision.MealFactory, _combineFactoryContoller);
            // Entity 와 Controller 연결

            _combineFactoryContoller.SetActPlayer(_actPlayer);
            _combineFactoryContoller.SetUp(uUID, _entityTID);
            // 컨트롤러 셋업

            _combineFactoryContoller.SetMoveAgent(_moveAgent);
            // 무빙 에이전트 셋업

            _moveAgent.SetPathFinder(_pathFinder);
            // Path Finder 셋업

            _moveAgent.OnInitialize(uUID, _combineFactoryContoller);
            // 필수 Entity Script 설정
            ...
            _combineFactoryContoller.AISetUp();
            // AI SetUp

            _modelObj.transform.SetParent(_obj.transform);
            // 부모로 세팅

            _combineFactoryContoller.Pos3D = _position;
            // 포지션 세팅
    ...
    public async UniTask CreateEntity(int _entityTID, Vector3 _position, Action<Entity> _onCB_Create)
    {
        ...
        ResourceManager.GetInstance().GetResource(ResourceType.FactoryEntity, _entityTID, true, (_modelResource) => {
            ...
            var _userContoller = _obj.AddComponent<EntityCombineFactoryController>();
...
/// 식당 공장을 만드는 생산 패턴
/// </summary>
public class StoreFactoryEntityFactory : EntityFactoryBase
{
    public override Entity CreateEntity(int _entityTID, Vector3 _position)
    {
        ResourceManager.GetInstance().GetResource(ResourceType.FactoryEntity, _entityTID, true, (_modelResource) => {
            ...
            var _mealFactoryContoller = _obj.AddComponent<EntityStoreFactoryController>();
    ...
    public async UniTask CreateEntity(int _entityTID, Vector3 _position, Action<Entity> _onCB_Create)
    {
        ...
        ResourceManager.GetInstance().GetResource(ResourceType.FactoryEntity, _entityTID, true, (_modelResource) => {
            ...
            var _userContoller = _obj.AddComponent<EntityStoreFactoryController>();
